...
WeightedSquareGraph::WeightedSquareGraph(float width_meters, float height_meters, float precision_meters)
    ...
    for (short x = 0; x < WIDTH_NODES; x++)
        for (short y = 0; y < HEIGHT_NODES; y++) edges[{x, y}] = {x * NODE_WIDTH_METERS, y * NODE_HEIGHT_METERS};
}

Vector2f WeightedSquareGraph::get_location(const Vector2i pos) { return edges.at(pos); }

Vector2i WeightedSquareGraph::snap_to_grid(const Vector2f& pos_meters) const {
    return {(short)round(pos_meters[0] / NODE_WIDTH_METERS), (short)round(pos_meters[1] / NODE_HEIGHT_METERS)};
}
...
void WeightedSquareGraph::remove_region(const Vector2f& bottom_left_meters, const Vector2f& top_right_meters) {
    ...
    for (auto x = BL_x; x <= TR_x; x++)
        for (auto y = BL_y; y <= TR_y; y++) edges.erase({x, y});
}
...
float WeightedSquareGraph::heuristic(const Vector2i& a, const Vector2i& b) const { return get_distance_meters(a, b); }
