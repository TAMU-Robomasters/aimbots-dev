    ...
    const float pitchAxisOffset)
{
    ...
    *turretYaw = atan2f(projectedTargetPosition.y, projectedTargetPosition.x);
    // *turretYaw = acos((projectedTargetPosition.y * sqrt(squaredTargetX + squaredTargetY - squaredBarrelPositionX) + BARREL_POSITION_FROM_GIMBAL_ORIGIN.getX() * projectedTargetPosition.x) / (squaredTargetX + squaredTargetY));
