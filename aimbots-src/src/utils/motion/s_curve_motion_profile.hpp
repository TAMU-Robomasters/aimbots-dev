...
public:
    float constrainVelocity() {
        ...
        return (sqrt(fabs(pow(maxAccel, 4.0) + 4.0 * displacement * maxAccel * maxJerkSquared)) - maxAccelSquared) /
               (maxJerk * 2.0);
    }

    SCurveMotionProfile(Constraints constraints, float displacement)
        : constraints(constraints),
          accelerationProfile(constraints),
    ...
    float timeAtDisplacement(float s) {
        ...
        return accelerationTime + cruiseTime + accelerationTime -
               accelerationProfile.timeAtDisplacement(accelerationDisplacement - s);
    }

    float totalTime() { return accelerationTime + accelerationTime + cruiseTime; }
};
