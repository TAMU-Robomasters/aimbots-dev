...
public:
    struct Step {
        // Derivatives of postion
        //  Used to determine how smoothly we want our robot to move
        //  Distance: 0
        //  Velocity: 1
        //  Acceleration: 2
        //  Jerk: 3
    ...
    struct Constraints {
        // Place limits on the derivatives (Higher limits are smoother)
        float velocity;
        float acceleration;
        float jerk;
...
public:
    ...
    Step stepAtT(float t) {
        ...
        if (t < t1) {
            return Step(
                constraints.jerk * (t * t * t) / 6.0f,
                constraints.jerk * (t * t) / 2.0,
                constraints.jerk * t,
                constraints.jerk);
        }
        ...
        if (t < t2) {
            return Step(
                d1 + v1 * t + constraints.acceleration * (t * t) / 2.0f,
                v1 + constraints.acceleration * t,
        ...
        if (t < t3) {
            return Step(
                d1 + d2 - constraints.jerk * (t * t * t) / 6.0f + constraints.acceleration * (t * t) / 2.0f + v2 * t,
                v2 + constraints.acceleration * t - (constraints.jerk * (t * t)) / 2.0f,
                constraints.acceleration - constraints.jerk * t,
                -constraints.jerk);
        }
    ...
    float timeAtDisplacement(float displacement) {
        ...
        if (displacement < d2) {
            float t = (-v1 + std::sqrt(v1 * v1 - 4.0f * constraints.acceleration / 2.0f * -displacement)) /
                      (constraints.acceleration);
            return (t + t1);
        }
    ...
    float totalTime() { return t1 + t2 + t3; }

    float totalDisplacement() { return d1 + d2 + d3; }
};
}  // namespace src::Utils::motion