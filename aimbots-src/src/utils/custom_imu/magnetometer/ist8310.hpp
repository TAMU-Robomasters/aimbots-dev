...
class Ist8310 : modm::I2cDevice<Ist8310Data::IST_I2C_MASTER> {
   ...
   private:
    inline modm::ResumableResult<bool> readRegister(Ist8310Data::Register reg, size_t length = 1) {
        ...
        transaction.configureWriteRead(raw_data_buffer, 1, raw_data_buffer, length);

        RF_END_RETURN_CALL(runTransaction());
    }

    inline modm::ResumableResult<bool> writeToRegister(Ist8310Data::Register reg, uint8_t* data, size_t length = 1) {
        RF_BEGIN();
        ...
        transaction.configureWrite(data, length);
