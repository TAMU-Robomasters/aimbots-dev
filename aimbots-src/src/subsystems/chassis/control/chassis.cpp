...
void ChassisSubsystem::updateMotorVelocityPID(WheelIndex WheelIdx, MotorOnWheelIndex MotorPerWheelIdx) {
    float err = 0;
    err = targetRPMs[WheelIdx][MotorPerWheelIdx] - (motors[WheelIdx][MotorPerWheelIdx]->getEncoder()->getVelocity() / static_cast<float>(M_TWOPI) * 60.0f);
    if (MotorPerWheelIdx == DRIVER) {
        err = targetRPMs[WheelIdx][MotorPerWheelIdx] - (motors[WheelIdx][MotorPerWheelIdx]->getEncoder()->getVelocity() / static_cast<float>(M_TWOPI) * 60.0f);
    } else if (MotorPerWheelIdx == YAW) {
        err = targetRPMs[WheelIdx][MotorPerWheelIdx] - motors[WheelIdx][MotorPerWheelIdx]->getEncoder()->getPosition().getWrappedValue();
        if (abs(err) > 4096) {
            int err_int =
...
void ChassisSubsystem::calculateSwerve(float x, float y, float r, float maxWheelSpeed) {
    ...
    if(x==0 && y==0 && r==0){
        ...
        right_front_yaw_actual = motors[RF][1]->getEncoder()->getPosition().getWrappedValue();
        ...
        left_back_yaw_actual = motors[LB][1]->getEncoder()->getPosition().getWrappedValue();
        ...
        right_back_yaw_actual = motors[RB][1]->getEncoder()->getPosition().getWrappedValue();
