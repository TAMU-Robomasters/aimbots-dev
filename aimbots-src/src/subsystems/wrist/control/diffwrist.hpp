...
enum MotorIndex { YAW, PITCH, ROLL }; // Really more of an "axis index"
...
public:
    ...
    bool isMotorOnline(uint8_t motorIdx) const { return motors[motorIdx].isMotorOnline(); }
...
private:
    std::array<std::array<DJIMotor, WRIST_MOTOR_COUNT_PER_SIDE>, > motors;
    std::array<SmoothPID, WRIST_MOTOR_COUNT_PER_SIDE> positionPIDs;
    std::array<SmoothPID, WRIST_MOTOR_COUNT_PER_SIDE> velocityPIDs;

    // The target desired angles of each motor AFTER scaling for the gear boxes
    std::array<float, WRIST_MOTOR_COUNT_PER_SIDE> targetAnglesRads{};
    std::array<float, WRIST_MOTOR_COUNT_PER_SIDE> targetRPMs{};
    std::array<float, WRIST_MOTOR_COUNT_PER_SIDE> desiredMotorOutputs{};
