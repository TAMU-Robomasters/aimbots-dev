...
void GimbalControlCommand::initialize() {
    ...
    static constexpr float xy_field_relative_patrol_location_array[15] = {
        -4.0f, 0.0f, 3000.0f, // field coordinate x, y, time spent at this angle
        -2.0f, 0.0f, 3000.0f,
        0.0f, 0.0f, 3000.0f,
        2.0f, 0.0f, 3000.0f,
        4.0f, 0.0f, 3000.0f,
...
void GimbalControlCommand::updateYawPatrolTarget() {
    ...
    if (controller->getYawPositionPID()->isSettled(2.0f)) {
        if (patrolTimer.execute()) {
            ...
            patrolTimer.restart(static_cast<uint32_t>(1500 /**patrolCoordinates.getRow(patrolCoordinateIndex)[2]*/));
        }
    }
}
...
float GimbalControlCommand::getFieldRelativeYawPatrolAngle(AngleUnit unit) {
    ...
    currPatrolCoordinateXDisplay = *currPatrolCoordinate[0];
    currPatrolCoordinateYDisplay = *currPatrolCoordinate[1];
    currPatrolCoordinateTimeDisplay = *currPatrolCoordinate[2];
