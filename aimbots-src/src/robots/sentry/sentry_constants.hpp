...
static const std::array<float, PITCH_MOTOR_COUNT> PITCH_MOTOR_OFFSET_ANGLES = {wrapTo0To2PIRange(modm::toRadian(104.6f))};
/* What motor angles ensures that the barrel is pointing straight forward and level relative to the robot chassis? */
...
static constexpr float PITCH_AXIS_SOFTSTOP_LOW = modm::toRadian(-24.5f);
static constexpr float PITCH_AXIS_SOFTSTOP_HIGH = modm::toRadian(13.0f);
