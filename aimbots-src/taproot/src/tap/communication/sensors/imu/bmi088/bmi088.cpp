...
void Bmi088::requestRecalibration(modm::Vector3f calibrationEulerAngles) {
    if (imuState == ImuState::IMU_NOT_CALIBRATED || imuState == ImuState::IMU_CALIBRATED) {
        ...
        lastCalibrationEulerAngles = calibrationEulerAngles;
        calibrationSample = 0;
...
void Bmi088::computeOffsets() {
    ...
    float cosx = cosf(lastCalibrationEulerAngles.getX());
    float sinx = sinf(lastCalibrationEulerAngles.getX());
    float cosy = cosf(lastCalibrationEulerAngles.getY());
    float siny = sinf(lastCalibrationEulerAngles.getY());
    float cosz = cosf(lastCalibrationEulerAngles.getZ());
    float sinz = sinf(lastCalibrationEulerAngles.getZ());

    float grav = -tap::algorithms::ACCELERATION_GRAVITY / ACC_G_PER_ACC_COUNT;

    float xGrav = (sinx * sinz - cosx * cosz * siny) * grav;
    float yGrav = (sinx * cosz + cosx * sinz * siny) * grav;
    float zGrav = (cosx * cosy) * grav;
    ...
    data.accOffsetRaw[ImuData::X] += data.accRaw[ImuData::X] + xGrav;
    data.accOffsetRaw[ImuData::Y] += data.accRaw[ImuData::Y] + yGrav;
    data.accOffsetRaw[ImuData::Z] += data.accRaw[ImuData::Z] + zGrav;
