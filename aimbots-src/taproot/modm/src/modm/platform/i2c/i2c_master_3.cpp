...
namespace
{
	...
	callStarting()
	{
		...
		if ((I2C3->SR2 & I2C_SR2_BUSY) and (nextOperation != modm::I2c::Operation::Restart))
		{
...
void
modm::platform::I2cMaster3::initializeWithPrescaler(uint8_t peripheralFrequency, uint8_t riseTime, uint16_t prescaler)
{
	// no reset, since we want to keep the transaction attached!

	Rcc::enable<Peripheral::I2c3>();
	...
	I2C3->FLTR = I2C_FLTR_ANOFF_Msk | 4;
# endif
...
void
modm::platform::I2cMaster3::reset()
{
	reading.length = 0;
	writing.length = 0;
...
// MARK: - ownership
bool
modm::platform::I2cMaster3::start(I2cTransaction *transaction, ConfigurationHandler handler)
{
	modm::atomic::Lock lock;
...
modm::platform::I2cMaster3::getErrorState()
{
	return error;
}