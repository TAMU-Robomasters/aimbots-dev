...
float OperatorInterface::getChassisXInput() {
    ...
    if (prevUpdateCounterX != updateCounter) {
        chassisXInput.update(drivers->remote.getChannel(tap::Remote::Channel::LEFT_HORIZONTAL), currTime);
...
float OperatorInterface::getChassisYInput() {
    ...
    if (prevUpdateCounterY != updateCounter) {
        chassisYInput.update(drivers->remote.getChannel(tap::Remote::Channel::LEFT_VERTICAL), currTime);
    ...
    // Scales analog values by values defined in standard_constants.hpp to speedshift input
    finalY *= drivers->remote.keyPressed(tap::Remote::Key::CTRL) ? CTRL_SCALAR : 1.0f;
    finalY *= drivers->remote.keyPressed(tap::Remote::Key::SHIFT) ? SHIFT_SCALAR : 1.0f;
...
float OperatorInterface::getChassisRotationInput() {
    ...
    if (prevUpdateCounterRotation != updateCounter) {
        chassisRotationInput.update(drivers->remote.getChannel(tap::Remote::Channel::RIGHT_HORIZONTAL), currTime);
    ...
    // Scales analog values by values defined in standard_constants.hpp to speedshift input
    finalRotation *= drivers->remote.keyPressed(tap::Remote::Key::CTRL) ? CTRL_SCALAR : 1.0f;
    finalRotation *= drivers->remote.keyPressed(tap::Remote::Key::SHIFT) ? SHIFT_SCALAR : 1.0f;
